...
ValuedAction POMCP::Search(double timeout) {
	...
	for (int i = 0; i < Globals::config.NUM_THREADS; i++) {
		...
		History local_history(history_);
		futures.push_back(
			async(launch::async, &ExpandTreeServerPOMCP, belief_, root_, model_, prior_, local_history,
...
void POMCP::ExpandTreeServerPOMCP(
	Belief* belief,
	Shared_VNode* root,const DSPOMDP* model,POMCPPrior* prior, History history,
	...
	if (root == NULL) {
		State* state = belief_->Sample(1)[0];
		root = CreateVNode(0, state, prior_, model_);
		static_cast<Shared_VNode*>(root)
		model_->Free(state);
	}
	...
	int num_sims = 0;
	while (true) {
		vector<State*> particles = belief->Sample(1000);
		for (int i = 0; i < particles.size(); i++) {
			State* particle = particles[i];
			logd << "[POMCP::Search] Starting simulation " << num_sims << endl;

			Simulate(particle, root, model, prior_);
